...
sidebar_position: 1
description: End-to-end guide for running a SIMPLE session — starting GAMA, preparing headsets, launching and monitoring a simulation.
---
...
:::tip
If a headset does not appear in the web platform after a few moments, check that its IP address is reachable and listed in `HEADSETS_IP` in your `.env`. See the [headset configuration guide](./meta-quest/headset-config.md).
:::
...
:::info
By default (`AGGRESSIVE_DISCONNECT = false` in `.env`), disconnected headsets stay visible in the panel so you can see them. They are not silently removed.
:::
