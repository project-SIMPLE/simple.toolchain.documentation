...
:::info
This path is configured by `LEARNING_PACKAGE_PATH` in your `.env`. See the [.env reference](/advanced/webplatform/env-reference) for details.
:::
...
Each VU folder **must** contain a `settings.json` file. See [Virtual Universe settings reference](../virtual-universes/settings.md) for the file format.
...
- [Understand the settings.json format](../virtual-universes/settings.md)
